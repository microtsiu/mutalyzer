...
def get_transcripts(accession, version, position):
    t_1 = time.time()
    reference = get_reference(accession, version)
    t_2 = time.time()
    print(len(reference.transcripts))

    position_s = int(position) - 100000
    position_e = int(position) + 100000

    transcripts = Transcript.query.filter_by(reference_id=reference.id).\
        filter(Transcript.transcript_start > position_s, Transcript.transcript_stop < position_e).all()
    t_3 = time.time()
